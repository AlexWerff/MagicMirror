...
angular.module('magicMirrorApp')
    .controller('VideosController', ['$rootScope', '$scope', '$http', '$window', 'YT_event', 'voiceService', VideosController])

function VideosController($rootScope, $scope, $http, $window, YT_event, voiceService) {
    //initial settings
    $scope.yt = {
        width: 600,
        height: 480,
    ...
    voiceService.addCommand("Play Video", function () {
        $scope.sendControlEvent(YT_event.PLAY);
    });

    voiceService.addCommand("Pause Video", function () {
        $scope.sendControlEvent(YT_event.PAUSE);
    });

    voiceService.addCommand("Next Video", function () {
        next();
    });

    voiceService.addCommand("Previous Video", function () {
        previous();
    });

    voiceService.addCommand("Search For *searchText", function (searchText) {
        search(searchText);
